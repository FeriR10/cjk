...
class PenjualanDealerPulsaController extends Controller
{
    public function index()
    {
        ...
        if (auth()->user()->role_id == 3) {
            $total_keuntungan = PenjualanDealerPulsa::with(['dealer_pulsa', 'pulsa', 'kartu'])->where('dealer_id', auth()->user()->dealer_id)->sum('keuntungan');
        }
        ...
        if (auth()->user()->role_id == 3) {
            $data_penjualan = PenjualanDealerPulsa::with(['dealer_pulsa', 'pulsa', 'kartu'])->where('dealer_id', auth()->user()->dealer_id)->get();
        }
